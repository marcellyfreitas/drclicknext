...
const specializationService = new EspecializacoesService(httpClient, '/public/especializacoes');

export function FormDialog({ open, onOpenChange, selectedId, onSuccess }: CreateFormPropsType) {
	const [medicoSearch, setMedicoSearch] = useState('');
	const [especializations, setEspecializations] = useState<Especializacao[]>([]);
	...
	const [loadingMedicosBySpecialization, setLoadingMedicosBySpecialization] = useState(false);
	...
	useEffect(() => {
		// Se médico foi selecionado, permite ir para a próxima tab
		if (watchedMedico && currentTab === 'profissional') {
			setCurrentTab('consulta');
		}
	}, [watchedMedico, currentTab]);

	useEffect(() => {
		// Quando uma especialização é selecionada, busca os médicos automaticamente
		if (selectedSpecialization) {
			fetchMedicosBySpecialization();
		} else {
			// Se nenhuma especialização está selecionada, limpa a lista de médicos
			setMedicoItems([]);
		}
	...
	const resetForm = () => {
		...
		setCurrentTab('profissional');
	};

	async function fetchSpecializations() {
		const response = await specializationService.getAll();
		const specializations = response.data.data;
		setEspecializations(specializations);
	}

	async function fetchMedicosBySpecialization() {
		if (!selectedSpecialization) return;

		setLoadingMedicosBySpecialization(true);
		try {
			...
			const medicos = collection.map((item: any) => ({
				value: item.id,
				label: item.name,
				subLabel: item.specialization?.name || 'Sem especialização',
			}));

			setMedicoItems(medicos);
		} catch (error) {
			console.error('Erro ao buscar médicos por especialização:', error);
			toast.error('Erro ao carregar médicos');
		} finally {
			setLoadingMedicosBySpecialization(false);
		}
	}
	...
	async function fetchMedicos(query: string): Promise<AutoCompleteItem<ItemType>[]> {
		...
		const response = await medicosService.getAll(params);
	...
	const handleSpecializationSelect = (specializationId: string) => {
		const newSelection = specializationId === selectedSpecialization ? '' : specializationId;
		setSelectedSpecialization(newSelection);
		setMedicoSearch('');
		setValue('medico', '');

		// Não precisa mais chamar fetchMedicosBySpecialization aqui porque o useEffect vai disparar
	};

	const canProceedToConsulta = () => {
		return !!watchedMedico;
	};
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-[600px]">
				<DialogHeader>
					...
					<DialogDescription>
						Selecione o profissional médico e depois os dados da consulta.
					</DialogDescription>
				</DialogHeader>

				<Tabs value={currentTab} onValueChange={setCurrentTab} className="w-full">
					<TabsList className="grid w-full grid-cols-2">
						<TabsTrigger value="profissional">Profissional Médico</TabsTrigger>
						<TabsTrigger
							value="consulta"
							disabled={!canProceedToConsulta()}
						>
							Dados da Consulta
						</TabsTrigger>
					</TabsList>
							...
							<div className="space-y-4">
									...
									<div className="flex flex-wrap gap-2">
										{especializations.map((especialization) => (
											<Badge
												...
												variant={selectedSpecialization === especialization.id ? 'default' : 'outline'}
												className="cursor-pointer px-3 py-1"
												onClick={() => handleSpecializationSelect(especialization.id)}
											>
								...
								<div className="space-y-2">
									<div className="flex items-center justify-between">
										...
										{loadingMedicosBySpecialization && (
									...
									<AutoComplete<ItemType>
										...
										onSearchValueChange={async (value) => {
											...
											if (!value) {
												// Se a busca está vazia e tem especialização selecionada, mostra todos os médicos da especialização
												if (selectedSpecialization) {
													return;
												}
												setMedicoItems([]);
										...
										placeholder={
											selectedSpecialization ?
												'Buscar médicos nesta especialização...' :
												'Selecione uma especialização ou busque médicos...'
										}
									...
									{selectedSpecialization && medicoItems.length === 0 && !loadingMedicosBySpecialization && (
										<p className="text-xs text-muted-foreground">
											Nenhum médico encontrado para esta especialização.
										</p>
									)}
								</div>
								...
								{watchedMedico && (
									<div className="bg-green-50 border border-green-200 rounded-md p-3">
										<p className="text-green-800 text-sm">
											✅ Médico selecionado. Avance para a próxima etapa.
										</p>
									</div>
								)}
							</div>
							...
							{!watchedMedico ? (
								<div className="bg-yellow-50 border border-yellow-200 rounded-md p-4 text-center">
									<p className="text-yellow-800 text-sm">
										Selecione um médico na etapa anterior para continuar.
									</p>
								</div>
							) : (
								<div className="space-y-4">
					...
					<div className="flex gap-2">
						...
						{currentTab === 'profissional' ? (
							<Button
								type="button"
								disabled={!canProceedToConsulta()}
								onClick={() => setCurrentTab('consulta')}
							>
								Próximo
							</Button>
						) : (
							<Button
								...
								disabled={isSubmitting || !canProceedToConsulta()}
								className="min-w-20"
							>
